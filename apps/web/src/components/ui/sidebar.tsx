...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      {...props}
    >
      {isExpanded ? <PanelLeftCloseIcon /> : <PanelLeftIcon />}
...
function SidebarGroupLabel({ className, render, ...props }: useRender.ComponentProps<"div">) {
  const defaultProps = {
    className: cn(
      "flex h-8 shrink-0 items-center rounded-lg px-2 font-medium text-sidebar-foreground text-xs outline-hidden ring-ring transition-none focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
